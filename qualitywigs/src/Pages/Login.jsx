  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col items-center w-[90%] sm:max-w-96 m-auto mt-14 gap-4 text-gray-800 '>
        ...
        <div className='w-full flex justify-between text-sm mt-[-8px]'>
          ...
          {
            currentState === 'Login' ? <p onClick={()=>setCurrentState('Register')} className='cursor-pointer'>Create an Account</p> : <p onClick={()=>setCurrentState('Login')} className='cursor-pointer'>Login</p>
          }
        </div>
        <button className='bg-black w-full text-white font-light px-8 py-2 mt-4'>{currentState === 'Login'? 'Log In' : 'Register'}</button>
    </form>
  )
}
